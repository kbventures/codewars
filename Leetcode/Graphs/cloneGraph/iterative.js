...
function cloneGraph(originalNode) {
    ...
    var visitedMap = new Map(); // Track visited nodes
    visitedMap.set(originalNode, new Node(originalNode.val)); // Clone initial node
    var queue = [originalNode]; // BFS queue initialized with original node
    while (queue.length > 0) {
        var currentNode = queue.shift(); // Dequeue node
        for (var _i = 0, _a = currentNode.neighbors; _i < _a.length; _i++) {
            var neighbor = _a[_i];
